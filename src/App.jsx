...
export default function App() {
  ...
  // Trigger burgundy splash and color transition immediately after the logo lands on a new page
  useEffect(() => {
    const timer = setTimeout(() => {
      const container = containerRef.current
      if (!container) return
      const activePageEl = container.querySelector(`#page-${page}`)
      if (!activePageEl) return
      const logoEl = activePageEl.querySelector('.couple-logo')
      if (!logoEl) return

      const pageRect = activePageEl.getBoundingClientRect()
      const logoRect = logoEl.getBoundingClientRect()

      const x = logoRect.left - pageRect.left + logoRect.width / 2
      const y = logoRect.top - pageRect.top + logoRect.height / 2

      setRippleData({ pageIndex: page, x, y })

      // Add painted class for color transition
      activePageEl.classList.add('painted')
      
      const removePaintTimer = setTimeout(() => {
        // Remove painted class after transition completes
        // Keep the burgundy color by adding a persistent class
        activePageEl.classList.add('active-burgundy')
        setTimeout(() => {
          activePageEl.classList.remove('painted')
        }, 300)
      }, 1200)

      const removeRippleTimer = setTimeout(() => {
        setRippleData(null)
      }, 1100)

      return () => {
        clearTimeout(removePaintTimer)
        clearTimeout(removeRippleTimer)
      }
    }, 320)
  ...
  // Cinematic Scroll-Driven Floating Logo Effect (Preserved Exactly)
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
  ...
  return <main className={`invitation-app ${keyboardOpen ? 'keyboard-open' : ''}`} ref={containerRef}>
    ...
    <style>{`
      .invitation-app{height:100vh;height:100dvh;overflow-y:auto;scroll-snap-type:y mandatory;background:#360817;color:#fff;scroll-behavior:smooth;position:relative}.invitation-app.keyboard-open{scroll-snap-type:none}.glitters-container{position:fixed;inset:0;pointer-events:none;z-index:5;overflow:hidden}.glitter{position:absolute;background:#ffe8d6;border-radius:50%;box-shadow:0 0 10px 2px #f4c2afaa;animation:floatAndTwinkle infinite ease-in-out}@keyframes floatAndTwinkle{0%{transform:translateY(0px) scale(0.7);opacity:0.15}50%{transform:translateY(-30px) scale(1.3);opacity:0.85}100%{transform:translateY(-60px) scale(0.7);opacity:0.15}}.page{min-height:100vh;min-height:100dvh;width:100%;box-sizing:border-box;display:flex;position:relative;overflow-y:auto;overflow-x:hidden;isolation:isolate;scroll-snap-align:start;scroll-snap-stop:always;background-color:#ffffff;transition:background-color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.cover-collapsible{max-height:420px;opacity:1;overflow:hidden;transition:max-height .45s cubic-bezier(.16,1,.3,1),opacity .3s ease,transform .45s cubic-bezier(.16,1,.3,1);transform:translate3d(0,0,0) scale(1);transform-origin:top center}.cover.keyboard-open .cover-collapsible{max-height:0;opacity:0;transform:translate3d(0,-18px,0) scale(.94);pointer-events:none}.cover .page-inner{transition:padding .45s cubic-bezier(.16,1,.3,1)}.cover.keyboard-open .page-inner{padding-top:24px;padding-bottom:280px}.couple-logo{width:110px;height:auto;display:block;margin:0 auto 12px;object-fit:contain;opacity:0;visibility:hidden;pointer-events:none;transition:filter 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.page-inner{width:min(1060px,100%);margin:auto;padding:clamp(45px,7vh,95px) 40px clamp(60px,8vh,95px);text-align:center;box-sizing:border-box;position:relative;border:2px double #e1a68e88;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1),backdrop-filter 0.3s ease,-webkit-backdrop-filter 0.3s ease}.page-inner::before{content:'';position:absolute;inset:10px;border:1px solid #e1a68e44;pointer-events:none}.page-inner::after{content:'❖ ❦ ❖';position:absolute;bottom:14px;left:50%;transform:translateX(-50%);color:#e1a68eaa;font-size:11px;letter-spacing:6px}.vintage-ornament{font-size:14px;color:#e1a68eaa;letter-spacing:8px;margin:6px 0;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.vintage-divider{display:flex;align-items:center;justify-content:center;margin:14px auto;color:#e1a68eaa;font-size:12px;letter-spacing:8px;width:220px}.vintage-divider::before,.vintage-divider::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,transparent,#e1a68e88,transparent);transition:background 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.cover .page-inner{display:flex;flex-direction:column;justify-content:flex-start}.cover-title{font:500 clamp(42px,7.5vw,95px)/.85 'Playfair Display';letter-spacing:-.07em;margin:10px 0 16px;color:#58122a;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.cover-title i{display:block;font-size:.55em;color:#e1a68e;font-weight:400}.cover-subtitle{font:12px 'DM Mono';letter-spacing:.2em;text-transform:uppercase;color:#58122a;margin:clamp(8px,2vh,20px) 0;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.lookup{width:min(470px,100%);position:relative;margin:clamp(10px,2vh,20px) auto;transition:transform .45s cubic-bezier(.16,1,.3,1)}.cover.keyboard-open .lookup{transform:translate3d(0,-4px,0)}.lookup input{width:100%;height:50px;border:1px solid #e9af9780;background:#26040e66;padding:0 20px;color:#fff;outline:0;font:14px 'DM Sans'}.lookup input::placeholder{color:#ffffffa1}.results{position:absolute;top:55px;width:100%;z-index:10;background:#fff;color:#4a2330;box-shadow:0 14px 40px #19030bbb;max-height:210px;overflow-y:auto;-webkit-overflow-scrolling:touch}.results button{width:100%;background:#fff;border:0;border-bottom:1px solid #eadbd4;padding:12px 18px;text-align:left;display:flex;justify-content:space-between;align-items:center}.results button:hover{background:#f9edeb}.results span{font:600 17px 'Playfair Display';color:#58122a}.results small{font:9px 'DM Mono';letter-spacing:.1em;text-transform:uppercase;color:#ae7569}.page-kicker{font:10px 'DM Mono';letter-spacing:.28em;text-transform:uppercase;color:#58122a;margin:0 0 clamp(6px,1.5vh,14px);transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.proposal-name{font:500 clamp(44px,7vw,92px)/1.15 'Playfair Display';color:#58122a;margin:10px 0 16px;letter-spacing:-.05em;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.proposal-role{font:500 clamp(32px,5vw,58px)/1.2 'Playfair Display';color:#e5a78e;margin:10px 0 clamp(12px,2vh,20px);transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.message{max-width:550px;margin:clamp(10px,2vh,20px) auto;line-height:1.7;font-size:14px;color:#58122a;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.details-page{background:#f7e9e2;color:#58122a;transition:background-color 0.8s cubic-bezier(0.16, 1, 0.3, 1),color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.details-page .page-inner{border-color:#c78d8077}.details-page .page-inner::before{border-color:#c78d8044}.details-page .page-inner::after{color:#a56559aa}.details-page .vintage-ornament,.details-page .vintage-divider{color:#a56559aa}.details-page .vintage-divider::before,.details-page .vintage-divider::after{background:linear-gradient(90deg,transparent,#c78d80aa,transparent)}.details-page .page-kicker{color:#a56559}.details-title{font:500 clamp(40px,6.5vw,78px)/.9 'Playfair Display';margin:0;color:#58122a;transition:color 0.8s cubic-bezier(0.16, 1, 0.3, 1)}.mini-events{margin:clamp(15px,3vh,30px) auto 0;display:grid;grid-template-columns:1fr 1fr;max-width:800px}.mini-event{padding:clamp(16px,2.5vh,28px) 20px;background:#fff;border:1px solid #ead5cd;position:relative}.mini-event:first-child{border-right:1px solid #ead5cd}.mini-event h3{font:600 clamp(18px,2.5vw,24px)/1.1 'Playfair Display';margin:8px 0;color:#58122a}.mini-event p{font-size:13px;line-height:1.6;color:#775a62}.mini-event a{display:inline-block;margin-top:10px;color:#58122a;font-size:10px;letter-spacing:.13em;text-transform:uppercase;text-decoration:none;border-bottom:1px solid #c6907b;padding-bottom:3px}.response-page h2{font:500 clamp(40px,6.5vw,78px)/.95 'Playfair Display';margin:0}.response-actions{display:flex;justify-content:center;gap:12px;margin-top:clamp(16px,3vh,28px)}.response-actions button,.modal-actions button{width:180px;min-height:46px;border:1px solid #dba087;background:#dba087;color:#3b0918;padding:12px;text-transform:uppercase;letter-spacing:.12em;font-size:10px;font-weight:600;cursor:pointer}.response-actions .decline-button,.modal-actions .decline-button{background:#eee5e2;border-color:#bca5a5;color:#947c80;cursor:not-allowed;opacity:.78}.modal-backdrop,.error-backdrop{position:fixed;inset:0;z-index:20;background:#250510bd;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:24px 16px;overflow-y:auto}.rsvp-modal,.error-dialog{width:min(450px,100%);margin:auto;max-height:85dvh;overflow-y:auto;background:#fff9f6;color:#58122a;padding:28px 24px;text-align:center;position:relative;box-shadow:0 18px 55px #1b030c80;border:2px double #c9aba2;box-sizing:border-box}.rsvp-modal h2,.error-dialog h2{font:600 32px/1 'Playfair Display';margin:6px 0 16px}.rsvp-modal label{text-align:left;display:block;font:10px 'DM Mono';letter-spacing:.12em;text-transform:uppercase;margin-top:12px}.rsvp-modal input,.rsvp-modal textarea{width:100%;border:0;border-bottom:1px solid #c9aba2;background:transparent;padding:8px 0;outline:0;font:14px 'DM Sans';color:#3f2830}.rsvp-modal textarea{min-height:56px;resize:vertical}.close{position:absolute;right:15px;top:8px;border:0;background:none;color:#58122a;font-size:27px;cursor:pointer}.modal-actions{display:flex;justify-content:center;gap:12px;margin-top:18px}.error-dialog{border-top:5px solid #a5213e}.error-icon{width:48px;height:48px;margin:auto;border-radius:50%;background:#a5213e;color:#fff;display:grid;place-items:center;font:600 30px 'DM Sans'}.error-dialog p{font-size:13px;line-height:1.7;color:#725962}.error-dialog button{width:100%;margin-top:16px;border:0;background:#58122a;color:#fff;padding:12px;text-transform:uppercase;letter-spacing:.13em;font-size:10px;cursor:pointer}.toast{position:fixed;z-index:25;bottom:80px;left:50%;transform:translateX(-50%);background:#fff;color:#58122a;padding:14px 22px;box-shadow:0 7px 28px #18030c55;font-size:13px}
      ...
      .floating-sparkle{will-change:transform,opacity;pointer-events:none}

      /* 🎨 BURGUNDY SPLASH - Starts from logo and spreads like liquid */
      .liquid-burgundy-ripple {
        position: absolute;
        border-radius: 50%;
        pointer-events: none;
        z-index: 20;
        ...
        background: radial-gradient(
          circle,
          rgba(88, 18, 42, 0.98) 0%,
          rgba(88, 18, 42, 0.92) 15%,
          rgba(140, 40, 70, 0.85) 35%,
          rgba(180, 60, 90, 0.6) 55%,
          rgba(200, 80, 110, 0.3) 75%,
          transparent 100%
        );
        box-shadow: 
          0 0 80px rgba(88, 18, 42, 0.6),
          inset 0 0 60px rgba(140, 40, 70, 0.4);
        animation: burgundyLiquidWave 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
      }
      
      @keyframes burgundyLiquidWave {
        0% {
          width: 0;
          height: 0;
          opacity: 1;
      ...
      /* Paint theme reveal - subtle texture as burgundy spreads */
      .page.painted .page-inner {
        animation: paintThemeReveal 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
      }
      
      @keyframes paintThemeReveal {
        0% {
      ...
      /* 🎨 Page Color Transition - White to Burgundy */
      /* Default: White pages */
      .page {
      ...
      .page .page-inner .cover-title,
      .page .page-inner .proposal-name,
      .page .page-inner .details-title {
        color: #58122a;
        transition: color 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }
      
      .page .page-inner .cover-subtitle,
      .page .page-inner .page-kicker {
        color: #58122a;
        transition: color 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }
      
      .page .page-inner .message {
        color: #58122a;
        transition: color 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }
      
      .page .page-inner .couple-logo {
        filter: none;
        transition: filter 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }
      
      .page .page-inner .vintage-ornament {
        color: #e1a68eaa;
        transition: color 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }
      
      .page .page-inner .vintage-divider {
        color: #e1a68eaa;
      }
      
      .page .page-inner .vintage-divider::before,
      .page .page-inner .vintage-divider::after {
        background: linear-gradient(90deg, transparent, #e1a68e88, transparent);
        transition: background 0.8s cubic-bezier(0.16, 1, 0.3, 1);
      }

      /* 🎨 Active Burgundy State (after burgundy splash) */
      .page.active-burgundy {
        background-color: #58122a !important;
      }
      
      .page.active-burgundy .page-inner {
        color: #f2dfd8;
      }
      
      .page.active-burgundy .page-inner .cover-title,
      .page.active-burgundy .page-inner .proposal-name,
      .page.active-burgundy .page-inner .details-title {
        color: #ffffff;
      }
      
      .page.active-burgundy .page-inner .proposal-role,
      .page.active-burgundy .page-inner .cover-subtitle,
      .page.active-burgundy .page-inner .page-kicker {
        color: #e1a68e;
      }
      
      .page.active-burgundy .page-inner .message {
        color: #e8d5ce;
      }
      
      .page.active-burgundy .page-inner .couple-logo {
        filter: brightness(0) invert(1);
      }
      
      .page.active-burgundy .page-inner .vintage-ornament {
        color: #e1a68eaa;
      }
      
      .page.active-burgundy .page-inner .vintage-divider::before,
      .page.active-burgundy .page-inner .vintage-divider::after {
        background: linear-gradient(90deg, transparent, #e1a68e88, transparent);
      }

      /* Details page specific transitions */
      .details-page.active-burgundy {
        background-color: #58122a !important;
      }
      
      .details-page.active-burgundy .page-inner {
        border-color: #e1a68e88;
      }
      
      .details-page.active-burgundy .page-inner::after {
        color: #e1a68eaa;
      }
      
      .details-page.active-burgundy .mini-event {
        background: rgba(255, 255, 255, 0.1);
        border-color: #e1a68e55;
      }
      
      .details-page.active-burgundy .mini-event h3 {
        color: #ffffff;
      }
      
      .details-page.active-burgundy .mini-event p {
        color: #e8d5ce;
      }
      
      .details-page.active-burgundy .mini-event a {
        color: #e1a68e;
        border-bottom-color: #e1a68e88;
      }

      /* Animation keyframes */
      .page.active .cover-subtitle,
      .page.active .page-kicker {
        animation: slideUp .7s .16s both;
      }
      .page.active .cover-title,
      .page.active .proposal-name,
      .page.active .details-title {
        animation: dramaticReveal .9s .25s cubic-bezier(.16,1,.3,1) both;
      ...
      .page.active .lookup,
      .page.active .mini-events,
      .page.active .response-actions,
      .page.active .proposal-role {
        animation: slideUp .8s .68s both;
      }
      .page.active .mini-event {
        animation: cardPop .7s both;
      }
      .page.active .mini-event:nth-child(2) {
        animation-delay:.14s;
      }
      .page.active .response-actions button {
        animation: buttonIn .65s .82s both;
      }
      .page.active .response-actions button:nth-child(2) {
        animation-delay:.94s;
      }
      ...
      .error-dialog button,
      .results button {
        transition:transform .2s cubic-bezier(.2,1.5,.5,1),box-shadow .2s,background .2s;
        position:relative;
        overflow:hidden;
      }
      .response-actions button:not(.decline-button):hover,
      .modal-actions button:not(.decline-button):hover,
      .error-dialog button:hover {
        transform:translateY(-5px) scale(1.04);
        box-shadow:0 12px 24px #13020a66;
      }
      .response-actions button:not(.decline-button):active,
      .modal-actions button:not(.decline-button):active,
      .error-dialog button:active {
        transform:scale(.93);
        box-shadow:none;
      }
      .response-actions button:not(.decline-button):before,
      .modal-actions button:not(.decline-button):before,
      .error-dialog button:before {
        content:'';
        position:absolute;
        inset:0;
        background:linear-gradient(105deg,transparent 30%,#ffffff9c 47%,transparent 64%);
        transform:translateX(-130%);
        transition:transform .55s;
      }
      .response-actions button:not(.decline-button):hover:before,
      .modal-actions button:not(.decline-button):hover:before,
      .error-dialog button:hover:before {
        transform:translateX(130%);
      }
      .decline-button:hover {
        animation:declineJitter .35s linear infinite;
        filter:saturate(1.25);
      }
      .results {
        animation:resultsDrop .35s cubic-bezier(.16,1,.3,1) both;
      }
      .results button {
        animation:resultSlide .4s both;
      }
      ...
      .results button:nth-child(4) {
        animation-delay:.15s;
      }
      .results button:nth-child(5) {
        animation-delay:.2s;
      }
      .results button:hover {
        transform:translateX(7px);
        box-shadow:-5px 0 #c6907b inset;
      }
      .modal-backdrop {
        animation:backdropIn .25s both;
      }
      .rsvp-modal {
        animation:modalBurst .62s cubic-bezier(.16,1.3,.3,1) both;
      }
      .error-backdrop {
        animation:warningFlash .45s both;
      }
      .error-dialog {
        animation:errorImpact .7s cubic-bezier(.18,1.55,.38,1) both;
      }
      .error-icon {
        animation:warningPulse 1.1s .4s infinite;
      }
      .toast {
        animation:toastFly .6s cubic-bezier(.16,1.2,.3,1) both;
      }
      .status-view {
        min-height:280px;
        display:flex;
        ...
        align-items:center;
        animation:statusFadeIn .45s cubic-bezier(.16,1,.3,1) both;
      }
      .status-view h3 {
        font:600 32px/1.1 'Playfair Display';
        color:#58122a;
        margin:10px 0;
      }
      .wedding-loader {
      ...
      .rings-container-wrapper {
        position:relative;
        display:inline-block;
        filter:drop-shadow(0 0 12px rgba(255,215,150,0.35));
      }
      .rings-animation-img {
        width:220px;
        height:auto;
        object-fit:contain;
        animation:ringMetallicShine 3s infinite ease-in-out;
      }
      .ring-glint {
        position:absolute;
        width:6px;
        ...
        background:#ffffff;
        border-radius:50%;
        box-shadow:0 0 10px 3px #ffffff, 0 0 20px 8px #ffd284;
        pointer-events:none;
        opacity:0;
      ...
      .ring-glint.glint-1 {
        top:20px;
        left:55px;
        animation:realisticGlint 2.2s infinite ease-in-out;
        animation-delay:0.2s;
      }
      .ring-glint.glint-2 {
        top:45px;
        right:45px;
        animation:realisticGlint 2.8s infinite ease-in-out;
        animation-delay:1.1s;
      }
      .ring-glint.glint-3 {
        bottom:25px;
        left:85px;
        animation:realisticGlint 2.5s infinite ease-in-out;
        animation-delay:0.6s;
      }
      @keyframes ringMetallicShine {
        0%,100%{filter:brightness(1) contrast(1.05)}
        50%{filter:brightness(1.22) contrast(1.12) drop-shadow(0 0 18px rgba(255,223,165,0.7))}
      }
      @keyframes realisticGlint {
        0%,100%{transform:scale(0.1) rotate(0deg);opacity:0}
        50%{transform:scale(1.4) rotate(45deg);opacity:0.95}
      }
      ...
      @keyframes dramaticReveal {
        from{opacity:0;transform:translate3d(0,35px,0) scale(.96);filter:blur(4px)}
      ...
      @keyframes cardPop {
        from{opacity:0;transform:perspective(700px) rotateX(35deg) translateY(25px)}
        to{opacity:1;transform:none}
      }
      @keyframes buttonIn {
      ...
      @keyframes dotPulse {
        50%{box-shadow:0 0 0 6px #e7ae9133}
      }
      ...
      @keyframes modalBurst {
        0%{opacity:0;transform:scale(.38) rotate(-5deg)}
        68%{transform:scale(1.05) rotate(1deg)}
        100%{opacity:1;transform:none}
      }
      @keyframes warningFlash {
        0%,100%{background:#250510bd}
        35%{background:#a5213ebd}
      }
      @keyframes errorImpact {
        0%{opacity:0;transform:scale(.15) rotate(-12deg)}
        50%{transform:scale(1.12) rotate(3deg)}
        72%{transform:scale(.96) rotate(-1deg)}
        100%{opacity:1;transform:none}
      }
      @keyframes warningPulse {
        50%{transform:scale(1.18);box-shadow:0 0 0 12px #a5213e2e}
      }
      @keyframes toastFly {
        from{opacity:0;transform:translate(-50%,45px) scale(.7)}
    ...
    <section id="page-0" data-index="0" className={`page cover ${page === 0 ? 'active' : ''} ${keyboardOpen ? 'keyboard-open' : ''}`}>
      <div className="page-inner">
        {rippleData && rippleData.pageIndex === 0 && (
          <div className="liquid-burgundy-ripple" style={{ left: `${rippleData.x}px`, top: `${rippleData.y}px` }} />
        )}
        <img 
          ref={coverLogoRef}
          src={logoLogo} 
    ...
    {guest && (
      <>
        <section id="page-1" data-index="1" className={`page proposal-page ${page === 1 ? 'active' : ''}`}>
          <div className="page-inner">
            {rippleData && rippleData.pageIndex === 1 && (
              <div className="liquid-burgundy-ripple" style={{ left: `${rippleData.x}px`, top: `${rippleData.y}px` }} />
            )}
            <img 
              src={logoLogo} 
        ...
        <section id="page-2" data-index="2" className={`page details-page ${page === 2 ? 'active' : ''}`}>
          <div className="page-inner">
            {rippleData && rippleData.pageIndex === 2 && (
              <div className="liquid-burgundy-ripple" style={{ left: `${rippleData.x}px`, top: `${rippleData.y}px` }} />
            )}
            <img 
              src={logoLogo} 
        ...
        <section id="page-3" data-index="3" className={`page thank-you-page ${page === 3 ? 'active' : ''}`}>
          <div className="page-inner">
            {rippleData && rippleData.pageIndex === 3 && (
              <div className="liquid-burgundy-ripple" style={{ left: `${rippleData.x}px`, top: `${rippleData.y}px` }} />
            )}
            <img 
              src={logoLogo} 
